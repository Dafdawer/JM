...
public class LastOne {
    public static void main(String[] args) {
        ...
        bb.get(1);
        // Random variables
        String randomFrom = "John Doe"; // Некоторая случайная строка. Можете выбрать ее самостоятельно.
        String randomTo = "Another JD";  // Некоторая случайная строка. Можете выбрать ее самостоятельно.
...
//   где по получателю можно получить список зарплат, которые были ему отправлены.
    ...
    public static class MailService <T> implements Consumer <Sendable<T>> {
        ...
        public void accept(Sendable <T> item) {
           if (mailBox.containsKey(item.getTo())) {
               List<T> list = mailBox.get(item.getTo());
               list.add(item.getContent());
               mailBox.put(item.getTo(), list);
           } else {
    ...
    public abstract static class Sendable<T>  {
